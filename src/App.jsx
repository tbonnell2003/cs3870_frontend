...
export const API_URL = "https://cs3870-backend-b6cu.onrender.com";
  ...
  return (
    <form onSubmit={handleSubmit} className="d-flex flex-column gap-3">
      <div>
        ...
        <input className="form-control" value={contact_name} onChange={(e) => setName(e.target.value)} required />
      </div>
      <div>
        <label className="form-label">Phone Number</label>
        <input className="form-control" value={phone_number} onChange={(e) => setPhone(e.target.value)} required />
      </div>
      <div>
        <label className="form-label">Message</label>
        <input className="form-control" value={message} onChange={(e) => setMessage(e.target.value)} required />
      </div>
      <div>
        <label className="form-label">Image URL</label>
        <input className="form-control" value={image_url} onChange={(e) => setImageUrl(e.target.value)} required />
      </div>
...
const DeleteContact = () => {
  ...
  const handleDelete = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`${API_URL}/contacts/${encodeURIComponent(name)}`, {
        method: "DELETE",
      });
...
const UpdateContact = () => {
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`${API_URL}/contacts/${encodeURIComponent(oldName)}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ contact_name, phone_number, message, image_url }),
      });
  ...
  return (
    <form onSubmit={handleUpdate} className="d-flex flex-column gap-3">
      <div>
        ...
        <input className="form-control" value={oldName} onChange={(e) => setOldName(e.target.value)} required />
      </div>
      <div>
